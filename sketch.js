...
function preload(){
    ...
    gameoverimage=loadImage("gameoverbutton.png");
    resetimage=loadImage("resetbutton.png");
    diesound=loadSound("die_sound.wav");
...
function setup() {
    ...
    gameover=createSprite(600,170,20,20);
    gameover.addImage(gameoverimage);
    gameover.scale=0.3;
    gameover.visible=false;
    resetbutton=createSprite(600,250,20,20);
...
if(slicesGroup.isTouching(baker)){
    ...
    scoresound.play();
        ...
        ground.velocityX=-(5+survival_time/200);
        if (ground.x < 0){
            ground.x = ground.width/2;
        }

        if(keyDown("space")&&baker.y>=320){
            baker.velocityY=-15;
            jumpsound.play();
        }
...
function spawnToppings(){
    if (frameCount % 100 === 0){
         var toppings = createSprite(displayWidth,360,10,40);
             toppings.velocityX = -(5+survival_time/200);
...
function spawnSlices(){
    if (frameCount % 235 === 0){
        ...
        slices.scale=0.2;
        slices.velocityX = -(5+survival_time/200); 
